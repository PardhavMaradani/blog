...
# square tiling
def sq_tiling(length, sx, sy, pattern):
  R = int(wh/(2 * length)) + 1
  C = int(ww/(2 * length)) + 1
  for r in range(R):
    y = sy + (2 * length * r)
    for c in range(C):
      x = sx + (2 * length * c)
      draw_tile(4, x, y, length, pattern)

# hexagon tiling
def hex_tiling(length, sx, sy, pattern):
  R = 2 * int(wh/(2 * length)) + 2
  C = int(ww/(2 * length)) + 1
  apothem = shape_apothem(3, length)
  for r in range(R):
    y = sy + (apothem * 3 * r)
    for c in range(C):
      if r % 2 == 0:
        ...
        x = sx + (length + (length / 2)) +  (length * 3 * c)
      draw_tile(3, x, y, length, pattern)
...
#sq_tiling(length, -ww/2, -wh/2, sq_pattern1)
#sq_tiling(length, -ww/2, -wh/2, sq_pattern2)
hex_tiling(length, -ww/2, -wh/2, tri_pattern1)
#hex_tiling(length, -ww/2, -wh/2, tri_pattern2)
