...
from _DataType import Point, Event, Arc, Segment, PriorityQueue
...
class Voronoi:
    ...
    def intersection(self, p0, p1, ll):
        ...
        if (p0.x == p1.x):
            ...
            c = 1.0 * (p0.y**2 + p0.x**2 - ll**2) / z0 - 1.0 * (p1.y**2 + p1.x**2 - ll**2) / z1

            py = 1.0 * (-b-math.sqrt(b*b - 4*a*c)) / (2*a)
            
        px = 1.0 * (p.x**2 + (p.y-py)**2 - ll**2) / (2*p.x-2*ll)
        res = Point(px, py)
        return res

    def finish_edges(self):
        ll = self.x1 + (self.x1 - self.x0) + (self.y1 - self.y0)
        i = self.arc
        while i.pnext is not None:
            if i.s1 is not None:
                p = self.intersection(i.p, i.pnext.p, ll*2.0)
                i.s1.finish(p)
            i = i.pnext
